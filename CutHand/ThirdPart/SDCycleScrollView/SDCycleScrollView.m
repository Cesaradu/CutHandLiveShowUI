...
- (void)setPlaceholderImage:(UIImage *)placeholderImage
{
    ...
    self.backgroundImageView.backgroundColor = [UIColor colorWithHexString:@"cccccc" alpha:1.0];
...
- (void)layoutSubviews
{
    ...
    if ([self.pageControl isKindOfClass:[TAPageControl class]]) {
        ...
        size = CGSizeMake(self.imagePathsGroup.count * 5 * 1.2, 5);
    }
    CGFloat x = (self.sd_width - size.width) * 0.5;
    if (self.pageControlAliment == SDCycleScrollViewPageContolAlimentRight) {
        x = self.mainView.sd_width - size.width - [self Suit:20];
    }
    CGFloat y = self.mainView.sd_height - size.height - [self Suit:20];
